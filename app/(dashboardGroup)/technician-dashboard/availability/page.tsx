      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card className="border-neutral-200 dark:border-neutral-800 shadow-sm">
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              <p className="text-xs text-neutral-500">Upcoming Active Days</p>
              <p className="text-lg font-bold text-neutral-900 dark:text-white">
                {sortedUpcomingDates.length} Days
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="upcoming" className="w-full">
        ...
        <TabsContent value="upcoming" className="space-y-4 outline-none">
          {isLoading ? (
            ...
            sortedUpcomingDates.map((dateStr) => (
              <Card
                key={dateStr}
                className="border-neutral-200 bg-white transition-colors dark:border-neutral-800 dark:bg-neutral-900 shadow-sm"
              >
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {upcomingGrouped[dateStr].length} Slots Available
                    </Badge>
                  </div>
                </CardHeader>
                ...
                <CardContent className="pt-0 space-y-4">
                  {(() => {
                    const daySlots = upcomingGrouped[dateStr].sort(
        ...
        <TabsContent value="history" className="space-y-4 outline-none">
          {isLoading ? (
            ...
            <Card className="border-neutral-200 border-dashed dark:border-neutral-800">
              <CardContent className="flex flex-col items-center justify-center p-12 text-center">
                <div className="mb-4 rounded-full bg-neutral-100 p-3 text-neutral-600 dark:bg-neutral-900/30 dark:text-neutral-400">
                ...
                <p className="mb-4 text-sm text-neutral-500 dark:text-neutral-400 max-w-sm">
                  You don't have any past booked appointments to show yet.
                </p>
              </CardContent>
            </Card>
          ) : (
            sortedHistoryDates.map((dateStr) => (
              <Card
                key={dateStr}
                className="border-neutral-200 bg-white transition-colors dark:border-neutral-800 dark:bg-neutral-900 shadow-sm opacity-90"
              >
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                  <div className="flex items-center gap-3">
                    <span className="text-base font-bold text-neutral-900 dark:text-white capitalize">
                    ...
                    {historyGrouped[dateStr]
                      ...
                      .map((slot) => {
                        ...
                        return (
                          <div
                            key={slot.id}
                            className="flex items-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-1.5 text-xs font-medium text-neutral-600 dark:border-neutral-800 dark:bg-neutral-900/50 dark:text-neutral-400"
                          >
